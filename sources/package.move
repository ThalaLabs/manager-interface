...
module meridian_manager::package {
    ...
    //                                                                        //
    const DEPLOYER_ADDRESS: address = @meridian_manager_deployer;                //
    const RESOURCE_ACCOUNT_SEED: vector<u8> = b"meridian_manager";               //
    const RESOURCE_ACCOUNT_ADDRESS: address = @meridian_manager;                 //
    //                                                                        //
    // Modules that can interface with the resource signer                    //
    friend meridian_manager::manager;                                            //
    //                                                                        //
    //                                                                        //
    // *************************************************************************
    ...
    /// Entry point to publishing new or upgrading modules AFTER initialization, gated by the MeridianManager
    ///
    /// ***NOTE***: Since this resource itself is the `MeridianManager`, we mark the manager as a friend and proxy calls
    /// to this through the manager to avoid circular dependencies. Other packages which copy this module should mark this
    /// as `public entry`, accepting a signer that is checked against `manager::is_authorized`.
    public(friend) fun publish_package(metadata_serialized: vector<u8>, code: vector<vector<u8>>) acquires ResourceSignerCapability {
        // *** From NOTE *** -- this check must be included when marked as a `public entry` function!
        // assert!(meridian_manager::manager::is_authorized(account), ERR_PACKAGE_UNAUTHORIZED);
