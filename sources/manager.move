module meridian_manager::manager {
    ...
    struct RoleV2 has key {
        ...
        /// meridian protocol manager can assign admin to any address
        /// meridian protocol manager can also set any role members
        admin: Option<address>,
    ...
    /// Initialize the Meridian Manager. We do not utilize an `init` module given the simplicity of this
    /// package. **MUST** be called from the original deployer account of this package.
    ///
    /// All manager operations of Meridian products are gated by `manager::is_authorized(&signer)`.
    ...
    public entry fun initialize(deployer: &signer, manager_address: address) {
        ...
        assert!(signer::address_of(deployer) == @meridian_manager_deployer, ERR_UNAUTHORIZED);
    ...
    /// Meridian protocol manager can always manage membership of any role
    ...
    public fun initialize_for_test(manager_address: address) {
        // In order for other modules to depend on MeridianManager and mock the manager in tests, we internally
    ...
    fun initialize_twice_err() {
        // prepare
        let deployer = account::create_account_for_test(@meridian_manager_deployer);
